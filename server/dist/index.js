...
io.on('connection', (socket) => {
    ...
    socket.on('get-project', (projectId) => {
    ...
    socket.on('join-project', (data) => {
        ...
        let projectId;
        let role = 'viewer'; // مقدار پیش‌فرض
        if (typeof data === 'string') {
            projectId = data;
            role = socket.handshake.query.role || 'viewer';
        }
        // بررسی اگر داده‌ها به صورت آبجکت باشد
        else if (data && typeof data === 'object') {
            projectId = data.projectId;
            role = data.role || socket.handshake.query.role || 'viewer';
        }
        else {
    ...
    socket.on('update-project-settings', (data) => {
    ...
    // درخواست شروع اسکرول
    socket.on('start-scrolling', (projectId) => {
        const project = projectManager.getProject(projectId);
        if (project) {
    ...
    // درخواست توقف اسکرول
    socket.on('stop-scrolling', (projectId) => {
        const project = projectManager.getProject(projectId);
        if (project) {
    ...
    // تنظیم موقعیت اسکرول
    socket.on('set-scroll-position', (data) => {
...
function updateViewerCount(projectId) {
